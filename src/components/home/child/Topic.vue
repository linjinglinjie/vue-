...
export default {
	...
	methods: {
		loadTop() {
			this.offset = 1;
			this.touchend = false;
			this.$http.post(baseUrl+'/bookLendingManagement/bookprofession/getPerfectSelection.shtml',this.transformRequest({label:'专题'}),{
                headers: {
		...
		loadMore() {
			...
			this.$http.post(baseUrl+'/bookLendingManagement/bookprofession/getPerfectSelection.shtml',this.transformRequest({label:'重磅推荐'}),{
                headers: {
                    'Content-Type': 'application/x-www-form-urlencoded;charset=utf-8'
                }
            }).then((response) => {
