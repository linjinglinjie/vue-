					...
					<section class="historyList" :class="{'browList': tabIndex == 0}" v-if="selectList.length != 0">
						<ul>
							<li v-for="(item, index) in selectList" :key="index">
								...
								<section class="des">
									<h1 class="bookName">{{item.book_name}}</h1><!--书名-->
									<p class="departmentname"><span>借书处:</span> {{item.borrow_department_name}}</p><!--借书处名称-->
									<p v-if="tabIndex == 0"><span>预计还书日期：</span></p>
									<p class="returnTime" v-if="tabIndex == 2">{{item.return_time}}</p><!--返还时间-->
									<p class="dateTime"><span>借书日期:</span> {{item.borrow_time}}</p><!--借书日期-->
...
export default {
	...
	methods: {
		_initDate() {
			...
			this.$http.get(baseUrl+'/bookLendingManagement/borrow/seach.shtml').then((response) => {
				this.historyList = response.data.page.list;
				this.changeId(0)
			}).catch(function(err){
			  console.log(err);
			});
		...
		_Borrow() {
			...
			for(let i=0;i<this.historyList.length;i++) {
				if(this.historyList[i].borrow_mode == 1) {
		...
		_reservation() {
			...
			this.selectList = [];
			for(let i=0;i<this.historyList.length;i++) {
				if(this.historyList[i].borrow_mode == 2) {
					if(this.selectList.length == 0){
						this.selectList[0] = this.historyList[i]
					}else {
						this.selectList = this.selectList.concat(this.historyList[i]);
		...
		_Return() {
			...
			this.selectList = [];
			for(let i=0;i<this.historyList.length;i++) {
				if(this.historyList[i].is_back != 0) {
					if(this.selectList.length == 0){
						this.selectList[0] = this.historyList[i]
					}else {
						this.selectList = this.selectList.concat(this.historyList[i]);
			...
			if(index == 0) {
				if(num == 0) {
					...
					this.selectList.sort(function(a,b){
						return Date.parse(b.borrow_time) - Date.parse(a.borrow_time);
					})
				}else if(num == 1) {
					let arr = [];
